...
function viewModule(moduleId) {
    ...
    if (!canAccess) {
        showAlert('error', 'Voce nao tem acesso a este conteudo.');
        return;
    }
    ...
    if (modalContent) {
        modalContent.innerHTML = `
            ...
            <div style="padding: 0 16px; max-height: 70vh; overflow-y: auto;">
                ${module.content}
...
function renderCycleCalendar() {
    ...
    for (let i = 0; i < 35; i++) {
        ...
        html += `<div style="width: 40px; height: 40px; background-color: ${color}; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 600; color: white;">${date.getDate()}</div>`;
    }

    html += '</div><div style="margin-top: 16px; font-size: 12px;"><div style="display: flex; gap: 8px; align-items: center; margin-bottom: 8px;"><div style="width: 20px; height: 20px; background-color: #ff6b9d; border-radius: 4px;"></div><span>Período</span></div><div style="display: flex; gap: 8px; align-items: center; margin-bottom: 8px;"><div style="width: 20px; height: 20px; background-color: #4ecdc4; border-radius: 4px;"></div><span>Ovulação</span></div><div style="display: flex; gap: 8px; align-items: center;"><div style="width: 20px; height: 20px; background-color: #ffe66d; border-radius: 4px;"></div><span>Período Fértil</span></div></div>';
...
function renderProfilePage() {
    ...
    if (!appState.isLoggedIn) {
        container.innerHTML = '<p class="text-muted">Faça login para ver seu perfil</p>';
        return;
    }
    ...
    if (sosContent) {
        sosContent.innerHTML = `
            <div class="alert alert-error mb-lg">
                <span>⚠️</span>
                <div><strong>Se você está em perigo, procure ajuda imediatamente!</strong></div>
            </div>
...
function showAlert(type, message) {
    ...
    alert.innerHTML = `<span>${type === 'success' ? '✓' : type === 'error' ? '✕' : 'ℹ️'}</span><div>${message}</div>`;
...
function getAgeGroupLabel(ageGroup) {
    const labels = { '1': '8-12 anos', '2': '13-15 anos', '3': '18+ anos' };
...
function showTerms() { showAlert('info', 'Termos de Serviço - Em desenvolvimento'); }
function showPrivacy() { showAlert('info', 'Política de Privacidade - Em desenvolvimento'); }
function showAbout() { showAlert('info', 'Sobre Nós - Em desenvolvimento'); }
function showFaq() { showAlert('info', 'FAQ - Em desenvolvimento'); }
function showContact() { showAlert('info', 'Contato - Em desenvolvimento'); }
